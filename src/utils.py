...
def prefilter_items(data, take_n_popular=5000, item_features=None):
    # Уберем популярные товары
    popularity = data.groupby('item_id')['user_id'].nunique().reset_index() / data['user_id'].nunique()
    popularity.rename(columns={'user_id': 'share_unique_users'}, inplace=True)
    top_popular = popularity[popularity['share_unique_users'] > 0.2].item_id.tolist()
    data = data[~data['item_id'].isin(top_popular)]
    # Уберем не популярные товары
...
def get_targets_sec_level(data, train, recommender, N):
    ...
    s = users_lvl_2.apply(
        lambda x: pd.Series(x['candidates']), axis=1).stack().reset_index(level=1, drop=True)
    s.name = 'item_id'
...
def extend_new_user_features(data, user_features, users_emb_df):
    ...
    user_features = user_features.merge(users_emb_df, how='left')# Эмбеддинги
    # Standart sale time
    ...
    # Age
    user_features['age'] = user_features['age_desc'].replace({'65+': 70, '45-54': 50, '25-34': 30, '35-44': 40, '19-24':20, '55-64':60})
    user_features = user_features.drop('age_desc', axis=1)
    # Income
    user_features['income'] = user_features['income_desc'].replace({'35-49K': 45,'50-74K': 70,'25-34K': 30,'75-99K': 95,'Under 15K': 15,
     '100-124K': 120,'15-24K': 20,'125-149K': 145,'150-174K': 170,'250K+': 250,'175-199K': 195,'200-249K': 245})
    ...
    # Средний чек, средний чек / неделя
...
def extend_new_item_features(data, item_features, items_emb_df):
    ...
    item_features = item_features.merge(items_emb_df, how='left')# Эмбеддинги
    # manufacturer
    ...
    # Среднее количество продаж товара в категории / неделя
    items_in_department = new_features.groupby('department')['item_id'].count().reset_index().sort_values('item_id', ascending=False)
    items_in_department.rename(columns={'item_id': 'items_in_department'}, inplace=True)
    sales_count_per_dep = new_features.groupby(['department'])['quantity'].count().reset_index().sort_values('quantity', ascending=False)
    ...
    # sub_commodity_desc
    items_in_department = new_features.groupby('sub_commodity_desc')['item_id'].count().reset_index().sort_values('item_id', ascending=False)
    items_in_department.rename(columns={'item_id': 'items_in_sub_commodity_desc'}, inplace=True)
    sales_count_per_dep = new_features.groupby(['sub_commodity_desc'])['quantity'].count().reset_index().sort_values('quantity', ascending=False)
